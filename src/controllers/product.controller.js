...
export const createProduct = async (req, res) => {
    ...
    try {
        ...
        const {nombre, descripcion, categoria, precio, stock, imagen, imagenDetalle } = req.body;

        if(!nombre || nombre.trim() === "" || nombre.length < 3) {
            return res.status(422).json({message:" El nombre es obligatorio y debe tener al menos 3 caracteres."})
        };

        if(!nombre || !descripcion || !categoria || !precio || !stock || !imagen || !imagenDetalle) {
            return res
                .status(422) 
...
export const updateProduct = async (req, res) => {
    ...
    try {
        ...
        if (typeof req.body.descripcion != "string") {
            return res.status(422).json({message: "La descripción debe ser un string"});
        };
